...
(() => {
  ...
  // Multiple overlays (mobile menu, reservation panel) can each want to
  // lock body scroll. A plain body.style.overflow = 'hidden' / '' from
  // each of them independently races: if the mobile menu's CTA both
  // closes the menu and opens the reservation panel on the same click,
  // whichever handler runs last wins and can leave scroll stuck locked
  // (or wrongly unlocked while an overlay is still open). A shared
  // counter makes "locked" mean "at least one overlay wants it locked".
  ...
  function runOpeningSequence() {
    ...
    // a load/error event. Generous ceiling since real photos on a slow
    // connection can legitimately take a while — this only exists to
    // guarantee the site is never permanently stuck, not to rush things.
