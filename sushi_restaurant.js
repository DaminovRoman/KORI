...
(() => {
  ...
  function runOpeningSequence() {
    ...
    // the page (and its buttons) forever.
    const forceHide = () => {
      opening.classList.add('is-hidden');
      opening.setAttribute('aria-hidden', 'true');
      body.classList.remove('is-loading');
    };
    window.setTimeout(forceHide, 6000);
    ...
    // Choreography (ms from load): frost spreads across the glass, then a
    ...
    // 2850 — full sequence begins its 900ms fade-out (mask lifts)
    // 3750 — hero content reveals, once the mask has fully cleared
    requestAnimationFrame(() => {
      ...
      // mask lifts — opening screen begins its 900ms fade-out
      setTimeout(() => {
        opening.classList.add('is-hidden');
        opening.setAttribute('aria-hidden', 'true');
        body.classList.remove('is-loading');
      ...
      // hero reveals shortly after the opening screen begins fading out.
      setTimeout(() => {
        triggerHeroReveal();
      }, 2850 + 250);
    });
